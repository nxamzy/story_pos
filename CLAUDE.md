...
- The launcher icon is generated, not hand-drawn: `python3 assets/icon/make_icon.py` draws the mark (a receipt with a torn edge and a barcode) into `assets/icon/icon.png` + `icon_foreground.png`, then `dart run flutter_launcher_icons` fans it out to `mipmap-*`, `drawable-*` and the iOS appiconset. **Do not pad `icon_foreground.png` yourself** — `flutter_launcher_icons` wraps the foreground in `android:inset="16%"`, so a self-padded mark ends up shrunk twice and disappears inside the icon; the script sizes the mark to ~90% of the canvas on purpose (`assets/icon/README.md` has the arithmetic). The tool also corrupts `ios/Runner.xcodeproj/project.pbxproj` on every run — hunting for `ASSETCATALOG_COMPILER_APPICON_NAME`, which the project-level Debug/Release configs don't have, it overwrites `ASSETCATALOG_COMPILER_GENERATE_SWIFT_ASSET_SYMBOL_EXTENSIONS = YES` with `= AppIcon`. The icon name is already set correctly on the target, so the edit is pure damage: `git checkout ios/Runner.xcodeproj/project.pbxproj` after every icon regeneration.
- `docs/` is the public website Play requires — `index.html` (privacy policy) and `delete-account.html` (deletion request), both bilingual and dependency-free. `firebase.json` now has a `hosting` block serving that folder, so `firebase deploy --only hosting --project storepost-a64b8` publishes them to `storepost-a64b8.web.app`; `**/*.md` and `play/**` are excluded from the deploy. `docs/play/` holds the store graphics (`icon-512.png`, `feature-graphic.png`, regenerated by `make_feature_graphic.py`) and `docs/play-listing.md` the store copy.
- Both public pages are live: **https://storepost-a64b8.web.app/** (privacy policy) and **/delete-account.html** (deletion request) — those are the two URLs Play asks for. Re-run the hosting deploy after editing anything in `docs/`.
- Still owned by the user, not the code: the upload keystore (`android/key.properties` is gitignored, so a fresh clone has no release signing), screenshots, and the Play Console Data safety / content-rating forms — see `android/RELEASE.md` §6.
