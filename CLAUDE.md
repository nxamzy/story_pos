...
- `SignUpBloc`-equivalent dead code has been removed. `presentation/onboarding/pages/onboarding_page.dart` (an alternate swipeable `PageView` onboarding built on `OnboardingBloc`) is still unused/unrouted — the three routed splash pages (`first_splash_page.dart` / `second_splash_page.dart` / `third_splash_page.dart`) are the live onboarding flow.
